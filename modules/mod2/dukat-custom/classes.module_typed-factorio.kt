...
external interface LuaBootstrap {
    ...
    fun <E : uint> on_event(event: E, f: ((data: Any) -> Unit)?, filters: Array<Any> = definedExternally)
    fun <E : uint> on_event(event: E, f: ((data: Any) -> Unit)?)
    fun <E : uint> on_event(event: Array<E>, f: ((data: Any) -> Unit)?)
...
external interface LuaChunkIterator : LuaIterable<ChunkPositionAndArea> {
    @nativeInvoke
...
external interface LuaGameScript {
    ...
    fun is_valid_sound_path(sound_path: ): Boolean
...
external interface LuaRendering {
    ...
    fun set_render_layer(id: uint64, render_layer: )
