...
internal fun FactorioModPlugin.PluginContext.configureModSettings(): Unit = with(settings) {
...
private fun FactorioModPlugin.PluginContext.configureLocalDev(): Unit = with(settings) {
...
//    serverConnectHost.convention("localhost")
    ...
    clientModsDirectory.convention(layout.file(
      currentOs.flatMap { os ->
        requireNotNull(os) // remove when Gradle supports JSpecify https://github.com/gradle/gradle/issues/24767
        ...
        when (os) {
          ...
          OS.MacOS -> macApplicationSupportDir.map { appSupport ->
